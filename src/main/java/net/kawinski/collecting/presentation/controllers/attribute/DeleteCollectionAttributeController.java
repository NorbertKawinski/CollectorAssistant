...
@Model
@Slf4j
public class DeleteCollectionAttributeController {
    ...
    @Inject
    public DeleteCollectionAttributeController(AccessController accessController,
        ...
        try (final NkTrace trace = NkTrace.info(log)) {
            ...
            attribute = collectionAttributeService.findByIdOrThrow(form.getId());
    ...
    public void confirm() {
        try (final NkTrace trace = NkTrace.info(log)) {
            collectionAttributeService.delete(attribute);

            Messages.addFlashGlobalInfo("Pomyślnie usunięto atrybut '" + attribute.getAttributeTemplate().getName() + "'.");
            JsfRedirectHelper.redirectReadCollection(attribute.getCollection());
        }
    }
}
